...
import { Film, PlayCircle, ExternalLink, Copy, Check, Tv, Database } from "lucide-react";
...
export function MediaShowcase({ data, lang = "en" }: { data: PortfolioData; lang?: Language }) {
  ...
  return (
    <div id="media-showcase-container" className="w-full flex flex-col gap-6 text-left">
      {/* Header section */}
      <div id="media-header" className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
        ...
        <div id="media-tabs" className="flex items-center gap-1.5 p-1 bg-zinc-900 border border-zinc-800 rounded-lg overflow-x-auto no-scrollbar max-w-full">
          <button
            id="media-tab-highlights"
            onClick={() => setActiveMediaTab("highlights")}
            className={`px-3.5 py-2.5 text-xs font-bold rounded-md uppercase tracking-wider transition-all flex items-center gap-1.5 flex-shrink-0 whitespace-nowrap min-h-[40px] ${
